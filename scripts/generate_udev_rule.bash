...
usage() {
    info "Usage: $0 -i <can_interface> [-n <desired_name>] [-b <bitrate>] [-h]"
    echo ""
    echo "Options:"
...
if [ -z "$DESIRED_NAME" ]; then
    ...
    info "No custom name specified, keeping original name: $DESIRED_NAME"
fi
...
# Save the udev rule
info "Saving udev rule to $UDEV_RULE_FILE"
